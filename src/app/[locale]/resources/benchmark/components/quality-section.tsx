...
const QualityChart = dynamic(
  () => import('./quality-chart'),
  { ssr: false }
)
...
export function QualitySection({ isRTL = false }: QualitySectionProps) {
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-3xl font-bold text-dark-dark-grey dark:text-white-pure mb-2">
            {isRTL ? 'جودة المخرجات' : 'Output Quality'}
          </h2>
          <p className="text-light-hh-grey dark:text-light-grey max-w-3xl">
            {isRTL 
              ? 'تحليل مقارن لجودة المخرجات بين البرومبتات الأساسية والمتقدمة، مع تقييم المستخدمين على مقياس من 5 نقاط.'
              : 'Comparative analysis of output quality between basic and pro prompts, with user ratings on a 5-point scale.'}
          </p>
        </motion.div>
            ...
            <Card className="bg-light-grey-light/30 dark:bg-dark/30 backdrop-blur-sm border border-light-high-grey/20 dark:border-high-grey/20 h-full">
              <CardContent className="p-5">
                <h3 className={cn(
                  ...
                  {isRTL ? 'تحليل الجودة' : 'Quality Analysis'}
                </h3>
                <QualityChart isRTL={isRTL} />
              </CardContent>
            </Card>
          </motion.div>
                ...
                <Card className="bg-light-grey-light/30 dark:bg-dark/30 backdrop-blur-sm border border-light-high-grey/20 dark:border-high-grey/20 flex-1">
                  <CardContent className={cn(
                    ...
                    <h3 className="text-lg font-semibold text-dark-dark-grey dark:text-white-pure mb-3">
                      {isRTL ? 'مقارنة الجودة' : 'Quality Comparison'}
                    </h3>
                    <div className="flex flex-col gap-3 mt-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-dark-dark-grey dark:text-white-pure font-medium text-sm">3.7/5</span>
                      </div>
                      <div className="w-full bg-light-high-grey/20 dark:bg-high-grey/20 rounded-full h-2">
                        <motion.div 
                          ...
                          initial="hidden"
                          animate={cardsInView ? "visible" : "hidden"}
                          className="bg-amber-400 dark:bg-amber-400 h-2 rounded-full"
                        ></motion.div>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <span className="text-dark-dark-grey dark:text-white-pure font-medium text-sm">4.2/5</span>
                      </div>
                      <div className="w-full bg-light-high-grey/20 dark:bg-high-grey/20 rounded-full h-2 mb-2">
                        <motion.div 
                          ...
                          initial="hidden"
                          animate={cardsInView ? "visible" : "hidden"}
                          className="bg-amber-600 dark:bg-amber-600 h-2 rounded-full"
                        ></motion.div>
                      </div>
                    </div>
                  </CardContent>
              ...
              >
                <Card className="bg-light-grey-light/30 dark:bg-dark/30 backdrop-blur-sm border border-light-high-grey/20 dark:border-high-grey/20 flex-1">
                  <CardContent className={cn(
                    ...
                    <h3 className="text-lg font-semibold text-dark-dark-grey dark:text-white-pure mb-3">
                      {isRTL ? 'عوامل تحسين الجودة' : 'Quality Improvement Factors'}
                    </h3>
                    <p className="text-light-hh-grey dark:text-light-grey text-sm mb-4">
                      {isRTL 
                        ? 'العوامل الرئيسية التي تساهم في تحسين جودة المخرجات:'
                        : 'Key factors contributing to output quality improvement:'}
                    </p>
                    <div className={cn(
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-amber-500 dark:bg-amber-500"></div>
                        <span className="text-dark-dark-grey/90 dark:text-white-pure/90 text-sm">
                          {isRTL ? 'تحسين بنية البرومبتات' : 'Enhanced prompt structure'}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-amber-500 dark:bg-amber-500"></div>
                        <span className="text-dark-dark-grey/90 dark:text-white-pure/90 text-sm">
                          {isRTL ? 'تخصيص حسب المجال' : 'Domain-specific customization'}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-amber-500 dark:bg-amber-500"></div>
                        <span className="text-dark-dark-grey/90 dark:text-white-pure/90 text-sm">
                          {isRTL ? 'تحسين معالجة اللغة' : 'Improved language processing'}
                        </span>
                      </div>
                    </div>
                  </CardContent>
