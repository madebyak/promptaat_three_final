...
export async function GET(
  ...
  try {
    ...
    // First fetch the prompt to check if it's premium
    try {
    ...
    // First get just the isPro status to check if we need subscription
    ...
    // Use our subscription guard to protect premium content
    return await handleProContentRequest(
      ...
      {
        errorMessage: "You need a Pro subscription to access this premium prompt",
        allowPreview: true
      }
    );
  } catch (error) {
