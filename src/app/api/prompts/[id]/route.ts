...
import { verifyProPromptAccess } from "@/lib/auth/subscription-guard"
...
export async function GET(
  ...
  try {
    ...
    // Get user session for subscription check and bookmark status
    let session;
    try {
    ...
    // Check if user has access to full prompt content
    try {
      prompt = await verifyProPromptAccess(prompt, session?.user?.id);
    } catch (accessError) {
      console.error('Error verifying prompt access:', accessError);
      // Continue with original prompt but log error
    }
